...
def create_postfix_str(start_time, end_time, style='ms', include_end_time=True):
    ...
    postfix_str = '_S' + str(int(1000*start_time)).zfill(7) + 'E' + str(int(1000*end_time)).zfill(7) + 'ms'
...
def process_collection(id):
    ...
    for ix, row in df.iterrows():
        ...
        if os.path.isfile(path):
            ...
            filename_new = filename + '_' + str(int(1000*start_time)) + 'To' + str(int(1000*end_time)) + 'ms'
