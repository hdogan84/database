...
'AVFRSPSP')
"""
tmp = ["r.collection_id = {}".format(x) for x in COLLECTION_IDS]
tmp = " or ".join(tmp)
collection_ids = "({}) and ".format(tmp)
...
WHERE
    a.background = 0 and
    {collections}
    s.olaf8_id IN {class_list}

""".format(
    collections=collection_ids, class_list=class_list
)
